...
export default function DonatePage({ params }: { params: Promise<{ needId: string }> }) {
    ...
    const handleSubmit = async (formData: { amount: number; donorBankNumber: string; proofFile: File | null }) => {
        ...
        try {
            ...
            const { lastHash } = await lastHashRes.json().catch(() => ({ lastHash: '0'.repeat(64) }));
            ...
            const txData = {
                id: txId,
                needId: resolvedParams.needId,
                donorId: user.id,
                donorBankNumber: formData.donorBankNumber,
                ...
                proofImageUrl: "mock-url" // Placeholder
            };
            ...
            router.push(`/donation-confirmation/${txId}?amount=${formData.amount}&hash=${hash}&ts=${timestamp}`);
        } catch (err) {
        ...
        <div className="min-h-screen bg-background text-foreground pb-20">
            ...
            <nav className="bg-card/80 backdrop-blur-2xl border-b border-border sticky top-0 z-50">
                <div className="max-w-3xl mx-auto px-6 h-20 flex items-center justify-between">
                    <Link href={`/needs/${resolvedParams.needId}`} className="flex items-center gap-3 font-black text-foreground hover:text-primary transition-all group">
                    ...
                    <span className="text-sm font-black tracking-tighter text-primary">IHSAN PAY</span>
                    <div className="w-8" />
                </div>
            </nav>
            ...
            <main className="max-w-xl mx-auto px-6 pt-12">
                <div className="mb-12 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20 mb-4">
                        ...
                        <span className="text-[8px] font-black uppercase tracking-[0.2em] text-primary">Cryptographic Verification Active</span>
                    </div>
                    <h1 className="text-3xl font-black tracking-tighter mb-4">Complete Donation</h1>
                    <p className="text-muted-foreground font-medium text-sm">
                        Supporting: <span className="text-foreground font-bold">{need?.title || 'Loading...'}</span>
                    </p>
                </div>
                ...
                <div className="mt-12 bg-card rounded-3xl p-8 border border-border flex gap-4 items-start">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground font-medium leading-relaxed">
                            Once you submit, a unique SHA-256 hash is generated.
                            Our validators will verify the bank transfer against your uploaded screenshot.
                            Upon approval, the transaction becomes an immutable part of our public ledger.
                        </p>
                    </div>
                </div>
            </main>
