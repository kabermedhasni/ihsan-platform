...
export async function GET() {
  ...
  try {
    ...
    const processedNeeds = data.map((need) => {
      ...
      const targetAmount = Number(need.amount_required);
      const isFullyFunded =
        (fundedAmount >= targetAmount || need.status === "completed") &&
      ...
      return {
        ...
        status: isFullyFunded ? "fullyFunded" : mapDbStatusToUi(need.status),
        partnerStatus: need.partner_status || "funded",
        createdAt: need.created_at,
        description: need.description,
        partner: need.partner_id ? "Assigned Partner" : "Pending Assignment",
      };
    });
