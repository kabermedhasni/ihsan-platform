...
export async function POST(request: Request) {
    ...
    if (role?.toLowerCase() !== "validator" && role?.toLowerCase() !== "admin") {
        return NextResponse.json({ error: "Forbidden: Only validators can create needs" }, { status: 403 });
    }

    try {
        const body = await request.json();
        const { title, description, amount_required, city, district, category, beneficiaries, partner_id } = body;
