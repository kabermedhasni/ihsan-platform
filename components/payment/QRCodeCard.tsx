    ...
    >
      ...
      <div className="text-center space-y-2 relative z-10">
        ...
        <p className="text-xs text-muted-foreground font-medium">
          Digital Cryptographic Proof
        </p>
      </div>

      <div className="bg-white p-6 rounded-4xl shadow-xl relative group/qr">
            ...
            <div className="grid grid-cols-8 grid-rows-8 gap-0.5 h-full w-full">
              {isMounted &&
                Array.from({ length: 64 }).map((_, i) => (
                  <div
                    key={i}
                    className={`bg-slate-900 ${Math.random() > 0.5 ? "opacity-100" : "opacity-0"}`}
                  />
                ))}
            </div>
          </div>
