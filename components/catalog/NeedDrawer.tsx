...
const NeedDrawer = ({
  ...
  const categoryKey =
    need?.category.toLowerCase().replace(/\s+/g, "") || "other";
      ...
      >
        {need && (
          <>
            ...
            <div className="p-6 flex-1 text-left rtl:text-right">
              ...
              <div className="bg-secondary/50 rounded-2xl p-5 mb-8 space-y-4 border border-border">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm text-muted-foreground mb-0.5">
                      {t("drawer.beneficiaries")}
                    </div>
                    <div className="font-medium text-foreground">
                      {t("drawer.beneficiariesCount", {
                        count: need.beneficiaries,
                      })}
                    </div>
                  </div>
                </div>
            ...
            {/* Footer CTA */}
            <div className="p-6 border-t border-border bg-card sticky bottom-0 z-10 flex flex-col gap-3">
              <Link
                href={`/needs/${need.id}`}
                className="w-full py-3 px-4 rounded-xl text-primary bg-primary/10 hover:bg-primary/20 font-bold text-sm transition-all text-center flex items-center justify-center gap-2"
              >
