...
export default function Timeline({ items }: TimelineProps) {
  ...
  return (
    <div className="bg-secondary/30 backdrop-blur-md p-8 rounded-3xl border border-white/5 relative">
      <div className="absolute top-8 bottom-8 left-13 w-px bg-white/10" />
      <div className="space-y-12">
        {items.map((item, idx) => (
          <div key={idx} className="relative flex gap-6 pl-4">
            ...
            <div className="space-y-1 pt-1">
              ...
              <h4 className="font-bold text-white text-left rtl:text-right">
                {item.title}
              </h4>
              <p className="text-xs text-muted-foreground font-bold lowercase text-left rtl:text-right">
                {item.status}
              </p>
            </div>
          </div>
        ...
        {items.length === 0 && (
          <p className="text-muted-foreground font-bold text-sm text-center py-10">
